...
export const Form = ({ handleCloseForm }) => {
  ...
  const { formValues, handleEmailChange, handleReportFrequency, emailError, reportNameError, handleSaveGroup, handleChangeDistribution, distributions, distributionSelected, onChangeReportTime,handleChangeReport, reportSelected, reports } = useForm(initialForm);
  ...
  return (
    <div className="fixed top-0 left-0 flex items-center justify-center w-full h-full bg-slate-200/50">
      <div className="w-full max-w-md bg-white shadow form__container">
        ...
        <PageTitle
          title={!!activeGroup ? 'Edit Details' : "Add Report Delivery"}
          stylesClass="text-center raleway-b text-3xl mb-2"
        />

        <PageDescription
          title={!!activeGroup ? 'You can edit details' : "Add a new Summary Report Delivery"}
